...
class SSHPipeHandler(object):
    ...
    def __init__(self, handler_id=None, term_messages=None, unpack=True,
        ...
        module_logger.addFilter(LoggerAddHostFilter())
    ...
    def message_queue_loop(self, ):
        ...
        while True:
            module_logger.debug("Getting from message_queue.")
            ...
            module_logger.debug("Message queue received: message: {}{}"
            ...
            if not visited_atstart:
                ...
                module_logger.debug("Message queue not visited atstart yet.")
                visited_atstart = True
                try:
                    self.atstart(message)
                except Exception as error:
                    module_logger.debug("Exception from atstart: {}."
                                        .format(error))
                    self._exitmsg = error
                    self._exitcode = 2
                    break
            ...
            try:
                ...
                module_logger.debug("Exception from handler: {}."
                                    .format(error))
                self._exitmsg = error
                self._exitcode = 3
                break
        ...
        try:
            ...
            module_logger.debug("Exception from atexit: {}.".format(error))
            self._exitmsg = error
            self._exitcode = 4
    ...
    def service_loop(self):
        ...
        module_logger.debug("Completed service loop; exitcode: {}; exitmsg:{}."
    ...
    def atstart(self, received):
        global module_logger
        module_logger.debug("atstart: nothing to do, message: {}."
                            .format(received))

    def handle(self, received):
        global module_logger
        module_logger.debug("handle: nothing to do, message: {}."
                            .format(received))

    def atexit(self, received):
        global module_logger
        module_logger.debug("atexit: with message {}".format(received))
        self.qlogger.stop()
        msg = received
